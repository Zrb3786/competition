...
def read_motion_extra_npz(path: Optional[str | Path]) -> Dict[int, Dict[str, np.ndarray]]:
    """Load standalone behavior-retardation / motion-intensity features.

    Expected NPZ keys:
      ids: [N]
      pair_mask: [N,4]
      motion_extra_pair: [N,4,Dp]
      motion_extra_stat: [N,Ds]
    """
    if not path:
        return {}
    p = Path(path)
    if not p.exists():
        print(f"[WARN] motion_extra npz not found: {p}")
        return {}
    data = np.load(str(p), allow_pickle=True)
    ids = data["ids"].astype(int).tolist()
    pair = data["motion_extra_pair"].astype(np.float32)
    stat = data["motion_extra_stat"].astype(np.float32)
    mask = data["pair_mask"].astype(np.float32) if "pair_mask" in data.files else np.ones(pair.shape[:2], dtype=np.float32)
    out: Dict[int, Dict[str, np.ndarray]] = {}
    for i, pid in enumerate(ids):
        out[int(pid)] = {"pair": pair[i], "mask": mask[i], "stat": stat[i]}
    return out


def read_gait_extra_npz(path: Optional[str | Path]) -> Dict[int, np.ndarray]:
    """Load standalone unit-aware gait summary features.
    ...
    p = Path(path)
    if not p.exists():
        print(f"[WARN] gait_extra npz not found: {p}")
        return {}
    data = np.load(str(p), allow_pickle=True)
    ids = data["ids"].astype(int).tolist()
    feat = data["gait_extra"].astype(np.float32)
    return {int(pid): feat[i] for i, pid in enumerate(ids)}
...
class ElderV3Dataset(Dataset):
    def __init__(
        ...
        motion_extra_map: Optional[Dict[int, Dict[str, np.ndarray]]] = None,
        gait_extra_map: Optional[Dict[int, np.ndarray]] = None,
        ...
        self.motion_extra_map = motion_extra_map or {}
        self.gait_extra_map = gait_extra_map or {}
    ...
    def _infer_dims(self) -> Dict[str, int]:
        ...
        for v in self.audio_big_map.values():
            ...
            break
        motion_extra_pair_dim = 0
        motion_extra_stat_dim = 0
        for v in self.motion_extra_map.values():
            motion_extra_pair_dim = int(v["pair"].shape[-1])
            motion_extra_stat_dim = int(v["stat"].shape[-1])
            break
        gait_extra_dim = 0
        for v in self.gait_extra_map.values():
            gait_extra_dim = int(len(v))
            break
        ...
        return {
            ...
            "motion_extra_pair_dim": motion_extra_pair_dim,
            "motion_extra_stat_dim": motion_extra_stat_dim,
            "gait_extra_dim": gait_extra_dim,
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        # Standalone behavior-retardation / motion-intensity features
        if dims.get("motion_extra_pair_dim", 0) > 0 or dims.get("motion_extra_stat_dim", 0) > 0:
            erec = self.motion_extra_map.get(pid)
            if erec is not None:
                motion_extra_pair = erec.get("pair", np.zeros((PAIR_COUNT, dims.get("motion_extra_pair_dim", 0)), dtype=np.float32)).astype(np.float32)
                motion_extra_mask = erec.get("mask", np.zeros(PAIR_COUNT, dtype=np.float32)).astype(np.float32)
                motion_extra_stat = erec.get("stat", np.zeros(dims.get("motion_extra_stat_dim", 0), dtype=np.float32)).astype(np.float32)
            else:
                motion_extra_pair = np.zeros((PAIR_COUNT, dims.get("motion_extra_pair_dim", 0)), dtype=np.float32)
                motion_extra_mask = np.zeros(PAIR_COUNT, dtype=np.float32)
                motion_extra_stat = np.zeros(dims.get("motion_extra_stat_dim", 0), dtype=np.float32)
            if motion_extra_pair.shape[-1] != dims.get("motion_extra_pair_dim", 0):
                tmp = np.zeros((PAIR_COUNT, dims.get("motion_extra_pair_dim", 0)), dtype=np.float32)
                tmp[..., : min(motion_extra_pair.shape[-1], tmp.shape[-1])] = motion_extra_pair[..., : min(motion_extra_pair.shape[-1], tmp.shape[-1])]
                motion_extra_pair = tmp
            if motion_extra_stat.shape[-1] != dims.get("motion_extra_stat_dim", 0):
                tmp = np.zeros(dims.get("motion_extra_stat_dim", 0), dtype=np.float32)
                tmp[: min(len(motion_extra_stat), len(tmp))] = motion_extra_stat[: min(len(motion_extra_stat), len(tmp))]
                motion_extra_stat = tmp
        else:
            motion_extra_pair = np.zeros((PAIR_COUNT, 0), dtype=np.float32)
            motion_extra_mask = np.zeros(PAIR_COUNT, dtype=np.float32)
            motion_extra_stat = np.zeros(0, dtype=np.float32)
        item["motion_extra_pair"] = torch.from_numpy(motion_extra_pair.astype(np.float32))
        item["motion_extra_pair_mask"] = torch.from_numpy(motion_extra_mask.astype(np.float32))
        item["motion_extra_stat"] = torch.from_numpy(motion_extra_stat.astype(np.float32))

        # Unit-aware gait summary features
        if dims.get("gait_extra_dim", 0) > 0:
            gait_extra = self.gait_extra_map.get(pid, np.zeros(dims.get("gait_extra_dim", 0), dtype=np.float32))
            if len(gait_extra) != dims.get("gait_extra_dim", 0):
                tmp = np.zeros(dims.get("gait_extra_dim", 0), dtype=np.float32)
                tmp[: min(len(gait_extra), len(tmp))] = gait_extra[: min(len(gait_extra), len(tmp))]
                gait_extra = tmp
        else:
            gait_extra = np.zeros(0, dtype=np.float32)
        item["gait_extra"] = torch.from_numpy(np.asarray(gait_extra, dtype=np.float32))
...
def build_model(dims: Dict[str, int], config: Any, *, for_predict: bool = False) -> nn.Module:
    ...
    if arch in {"v11_newA_no_cross", "v11_newa_no_cross", "v11_newA", "v11_newa"}:
        return DepFormerAVPV11AudioVG(
            ...
            use_pair_attn_cross=False, use_abig_v_fusion=True, use_official_av_fusion=False,
        )

    if arch in {"v11_allA_no_cross", "v11_alla_no_cross", "v11_allA", "v11_alla"}:
        return DepFormerAVPV11AudioVG(
            dims=dims, hidden_dim=hidden_dim, p_embed_bottleneck=p_embed_bottleneck,
            dropout=dropout, modality_dropout=modality_dropout,
            use_audio_big=True, use_p_extra=True, use_p_prior=True,
            use_pair_attn_cross=False, use_abig_v_fusion=True, use_official_av_fusion=True,
        )

    if arch in {"v11_newA_cross", "v11_newa_cross"}:
        return DepFormerAVPV11AudioVG(
            dims=dims, hidden_dim=hidden_dim, p_embed_bottleneck=p_embed_bottleneck,
            dropout=dropout, modality_dropout=modality_dropout,
            use_audio_big=True, use_p_extra=True, use_p_prior=True,
            use_pair_attn_cross=True, use_abig_v_fusion=True, use_official_av_fusion=False,
        )

    if arch in {"v11_allA_cross", "v11_alla_cross"}:
        return DepFormerAVPV11AudioVG(
            dims=dims, hidden_dim=hidden_dim, p_embed_bottleneck=p_embed_bottleneck,
            dropout=dropout, modality_dropout=modality_dropout,
            use_audio_big=True, use_p_extra=True, use_p_prior=True,
            use_pair_attn_cross=True, use_abig_v_fusion=True, use_official_av_fusion=True,
        )
...
class DepFormerAVPV10AudioPprior(nn.Module):
    ...
    def __init__(self, dims: Dict[str, int], hidden_dim: int = 96, p_embed_bottleneck: int = 48, dropout: float = 0.45,
        ...
        motion_extra_pair_dim = int(dims.get("motion_extra_pair_dim", 0))
        motion_extra_stat_dim = int(dims.get("motion_extra_stat_dim", 0))
        gait_extra_dim = int(dims.get("gait_extra_dim", 0))
        self.audio_branch = PairTemporalBranch(audio_dim, hidden_dim, dropout=dropout)
        self.audio_big_branch = PairFeatureBranch(audio_big_dim, hidden_dim, dropout=dropout)
        self.video_branch = PairTemporalBranch(video_dim, hidden_dim, dropout=dropout)
        self.gait_branch = SeqBranch(gait_dim, hidden_dim, dropout=dropout)
        self.gait_extra_branch = MLP(gait_extra_dim, hidden_dim, hidden_dim, dropout=dropout)
        self.motion_extra_pair_branch = PairFeatureBranch(motion_extra_pair_dim, hidden_dim, dropout=dropout)
        self.motion_extra_stat_branch = MLP(motion_extra_stat_dim, hidden_dim, hidden_dim, dropout=dropout)
        self.motion_extra_norm = nn.LayerNorm(hidden_dim)
        self.motion_stat_branch = MLP(motion_stat_dim, hidden_dim, hidden_dim, dropout=dropout)
        self.p_struct_branch = MLP(p_struct_dim, hidden_dim, hidden_dim, dropout=dropout)
        self.p_embed_branch = MLP(p_embed_dim, max(hidden_dim, p_embed_bottleneck * 2), p_embed_bottleneck, dropout=dropout)
        ...
        self.token_names = ["P", "Shared", "A_priv", "ABig_priv", "V_priv", "AV_priv", "G_priv", "GUnit_priv", "VBeh_priv", "MS_priv"]
        self.mod_emb = nn.Parameter(torch.randn(len(self.token_names), hidden_dim) * 0.02)
    ...
    def forward(self, batch: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        v_tok, v_pair = self.video_branch(batch["video"], batch["video_pair_mask"])
        g_tok = self.gait_branch(batch["gait"]); gunit_tok = self.gait_extra_branch(batch["gait_extra"]); ms_tok = self.motion_stat_branch(batch["motion_stat"])
        vbeh_pair_tok, _ = self.motion_extra_pair_branch(batch["motion_extra_pair"], batch["motion_extra_pair_mask"])
        vbeh_stat_tok = self.motion_extra_stat_branch(batch["motion_extra_stat"])
        vbeh_tok = self.motion_extra_norm(vbeh_pair_tok + vbeh_stat_tok)
        pair_mask_cross = batch["audio_pair_mask"] * batch["video_pair_mask"]
        av_tok = self.pair_pool(self.cross_fusion(a_pair, v_pair, pair_mask_cross), pair_mask_cross)
        aux = torch.stack([a_tok, abig_tok, v_tok, av_tok, g_tok, gunit_tok, vbeh_tok, ms_tok], dim=1)
        aux_mask = torch.ones((B, 8), dtype=torch.float32, device=device)
        aux_mask[:, 0] = (batch["audio_pair_mask"].sum(dim=1) > 0).float()
        ...
        aux_mask[:, 5] = 1.0 if self.gait_extra_branch.enabled else 0.0
        aux_mask[:, 6] = ((batch["motion_extra_pair_mask"].sum(dim=1) > 0).float() if self.motion_extra_pair_branch.enabled else 0.0)
        if self.motion_extra_stat_branch.enabled:
            aux_mask[:, 6] = 1.0
        aux_mask[:, 7] = 1.0 if self.motion_stat_branch.enabled else 0.0
...
class DepFormerAVPV11AudioVG(nn.Module):
    """v11 = v9_no_cross inheritance + PCA A_big + standalone VBeh/GUnit.

    Key variants are selected by build_model:
      - newA_only: use A_big as the only audio token; official audio can be disabled with --audio_features "".
      - allA: use official wav2vec/opensmile audio plus A_big.
      - no_cross: pair-level gated fusion only, matching the successful v9_no_cross setting.
      - cross: pair-level mutual attention on 4 pair tokens only; no frame-level cross attention.

    The model keeps P prior residual output:
      final logits = P_prior_logits + alpha * multimodal_residual_logits.
    """
    def __init__(
        self,
        dims: Dict[str, int],
        ...
        num_classes3: int = 3,
        use_audio_big: bool = True,
        use_p_extra: bool = True,
        use_p_prior: bool = True,
        use_pair_attn_cross: bool = False,
        use_abig_v_fusion: bool = True,
        use_official_av_fusion: bool = True,
        residual_init: float = 0.55,
    ) -> None:
        super().__init__()
        self.dims = dict(dims)
        self.hidden_dim = hidden_dim
        ...
        self.use_p_extra = use_p_extra and int(dims.get("p_extra_dim", 0)) > 0
        self.use_p_prior = use_p_prior
        self.use_abig_v_fusion = use_abig_v_fusion and self.use_audio_big
        self.use_official_av_fusion = use_official_av_fusion and int(dims.get("audio_dim", 0)) > 0

        p_struct_dim = int(dims.get("p_struct_dim", 0))
        p_embed_dim = int(dims.get("p_embed_dim", 0))
        p_extra_dim = int(dims.get("p_extra_dim", 0)) if self.use_p_extra else 0
        audio_dim = int(dims.get("audio_dim", 0))
        audio_big_dim = int(dims.get("audio_big_dim", 0)) if self.use_audio_big else 0
        video_dim = int(dims.get("motion_dim", 0)) + int(dims.get("official_video_dim", 0))
        motion_stat_dim = int(dims.get("motion_stat_dim", 0))
        gait_dim = int(dims.get("gait_dim", 0))
        motion_extra_pair_dim = int(dims.get("motion_extra_pair_dim", 0))
        ...
        # v9_no_cross success is preserved by use_attn=False.
        # cross variants set use_attn=True, but only over 4 pair tokens.
        self.official_av_fusion = PairMutualCrossFusion(hidden_dim, dropout=dropout, use_attn=use_pair_attn_cross)
        self.abig_v_fusion = PairMutualCrossFusion(hidden_dim, dropout=dropout, use_attn=use_pair_attn_cross)
        self.pair_pool = MaskedAttentionPool(hidden_dim)
        ...
        self.shared_pool = MaskedAttentionPool(hidden_dim)
        self.private_gate = nn.Sequential(
            nn.LayerNorm(hidden_dim * 2), nn.Linear(hidden_dim * 2, hidden_dim), nn.GELU(), nn.Dropout(dropout),
            nn.Linear(hidden_dim, hidden_dim), nn.Sigmoid()
        )

        # Private token order is fixed; inactive branches are masked out.
        self.token_names = [
            "P", "Shared", "A_priv", "ABig_priv", "V_priv", "AV_priv", "ABigV_priv",
            "G_priv", "GUnit_priv", "VBeh_priv", "MS_priv"
        ]
        self.mod_emb = nn.Parameter(torch.randn(len(self.token_names), hidden_dim) * 0.02)
        enc_layer = nn.TransformerEncoderLayer(
            d_model=hidden_dim,
        ...
        self.metric_proj = nn.Sequential(
            nn.LayerNorm(hidden_dim), nn.Dropout(dropout), nn.Linear(hidden_dim, hidden_dim), nn.GELU(),
        ...
        self.p_head2 = CosineClassifier(hidden_dim, 2, init_scale=10.0)
        self.reg = nn.Linear(hidden_dim, 1)
        self.p_reg = nn.Linear(hidden_dim, 1)
        residual_init = max(1e-4, min(0.95, float(residual_init)))
        self.residual_alpha_logit = nn.Parameter(torch.tensor(math.log(residual_init / (1.0 - residual_init)), dtype=torch.float32))
    ...
    def _shared_private(self, aux: torch.Tensor, mask: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        shared = self.shared_pool(aux, mask)
        shared_expand = shared.unsqueeze(1).expand_as(aux)
        g = self.private_gate(torch.cat([aux, shared_expand], dim=-1))
        return shared, aux - 0.35 * g * shared_expand
    ...
    def forward(self, batch: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        p_parts = [self.p_struct_branch(batch["p_struct"]), self.p_embed_branch(batch["p_embed"])]
        if self.p_extra_branch is not None:
            p_parts.append(self.p_extra_branch(batch["p_extra"]))
        p_tok = self.p_comb(torch.cat(p_parts, dim=-1))

        a_tok, a_pair = self.audio_branch(batch["audio"], batch["audio_pair_mask"])
        abig_tok, abig_pair = self.audio_big_branch(batch["audio_big"], batch["audio_big_pair_mask"])
        v_tok, v_pair = self.video_branch(batch["video"], batch["video_pair_mask"])
        g_tok = self.gait_branch(batch["gait"])
        gunit_tok = self.gait_extra_branch(batch["gait_extra"])
        ms_tok = self.motion_stat_branch(batch["motion_stat"])
        ...
        av_mask = batch["audio_pair_mask"] * batch["video_pair_mask"]
        abigv_mask = batch["audio_big_pair_mask"] * batch["video_pair_mask"]
        av_pair = self.official_av_fusion(a_pair, v_pair, av_mask)
        abigv_pair = self.abig_v_fusion(abig_pair, v_pair, abigv_mask)
        av_tok = self.pair_pool(av_pair, av_mask)
        abigv_tok = self.pair_pool(abigv_pair, abigv_mask)

        aux = torch.stack([a_tok, abig_tok, v_tok, av_tok, abigv_tok, g_tok, gunit_tok, vbeh_tok, ms_tok], dim=1)
        aux_mask = torch.ones((B, 9), dtype=torch.float32, device=device)
        aux_mask[:, 0] = (batch["audio_pair_mask"].sum(dim=1) > 0).float() if self.audio_branch.enabled else 0.0
        aux_mask[:, 1] = (batch["audio_big_pair_mask"].sum(dim=1) > 0).float() if self.audio_big_branch.enabled else 0.0
        aux_mask[:, 2] = (batch["video_pair_mask"].sum(dim=1) > 0).float() if self.video_branch.enabled else 0.0
        aux_mask[:, 3] = (av_mask.sum(dim=1) > 0).float() if self.use_official_av_fusion else 0.0
        aux_mask[:, 4] = (abigv_mask.sum(dim=1) > 0).float() if self.use_abig_v_fusion else 0.0
        aux_mask[:, 5] = 1.0 if self.gait_branch.enabled else 0.0
        aux_mask[:, 6] = 1.0 if self.gait_extra_branch.enabled else 0.0
        aux_mask[:, 7] = ((batch["motion_extra_pair_mask"].sum(dim=1) > 0).float() if self.motion_extra_pair_branch.enabled else 0.0)
        if self.motion_extra_stat_branch.enabled:
            aux_mask[:, 7] = 1.0
        aux_mask[:, 8] = 1.0 if self.motion_stat_branch.enabled else 0.0
        ...
        p_h = self.p_prior_proj(p_tok)
        res3, res2 = self.res_head3(h), self.res_head2(h)
        p3, p2 = self.p_head3(p_h), self.p_head2(p_h)
        alpha = torch.sigmoid(self.residual_alpha_logit) if self.use_p_prior else 1.0
        logits3 = p3 + alpha * res3 if self.use_p_prior else res3
        logits2 = p2 + alpha * res2 if self.use_p_prior else res2
...
def train_one_model(
    ...
    motion_extra_map: Optional[Dict[int, Dict[str, np.ndarray]]],
    gait_extra_map: Optional[Dict[int, np.ndarray]],
    fold: int,
    seed: int,
    exp_dir: Path,
) -> Dict[str, Any]:
    ...
    train_ds = ElderV3Dataset(
        ...
        audio_big_map=audio_big_map, p_extra_map=p_extra_map, motion_extra_map=motion_extra_map, gait_extra_map=gait_extra_map, scalers=scalers,
...
def train_command(args: argparse.Namespace) -> None:
    ...
    motion_extra_map = read_motion_extra_npz(cfg.motion_extra_npz)
    gait_extra_map = read_gait_extra_npz(cfg.gait_extra_npz)

    print(f"[INFO] train samples={len(df)} label3={df['label3'].value_counts().to_dict()} label2={df['label2'].value_counts().to_dict()}")
    print(f"[INFO] P_struct={len(p_map)} cols={len(p_cols)} P_embed={len(p_embed_map)} motion={len(m_map)} audio_big={len(audio_big_map)} p_extra={len(p_extra_map)} cols={len(p_extra_cols)} motion_extra={len(motion_extra_map)} gait_extra={len(gait_extra_map)}")
    ...
    for fold, (tr_idx, va_idx) in enumerate(skf.split(ids_all, y), start=1):
        ...
        for seed in seeds:
            res = train_one_model(cfg, train_rows, val_rows, p_map, p_embed_map, m_map, audio_big_map, p_extra_map, motion_extra_map, gait_extra_map, fold, seed, exp_dir)
...
def predict_command(args: argparse.Namespace) -> None:
    ...
    motion_extra_map = read_motion_extra_npz(getattr(args, "motion_extra_npz", ""))
    gait_extra_map = read_gait_extra_npz(getattr(args, "gait_extra_npz", ""))
    ...
    for p in tqdm(ckpt_paths, desc="predict checkpoints"):
        pred = predict_one_checkpoint(
            ...
            motion_extra_map=motion_extra_map, gait_extra_map=gait_extra_map,
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--motion_extra_npz", default="")
    p.add_argument("--gait_extra_npz", default="")
    p.add_argument("--model_arch", default="lite", choices=['lite', 'p_anchor_v4', 'anchor', 'p_anchor', 'v5_hier_ord', 'v5', 'hier_ord', 'v6_cross_motion', 'v6_cross', 'cross_motion', 'v6_cross_no_pgate', 'v6_no_pgate', 'v6_cross_no_crossgate', 'v6_no_crossgate', 'v7_res_hier', 'v7_hier', 'v7', 'v9_depformerv2', 'v9', 'depformerv2', 'v9_no_pguide', 'v9_no_p', 'v9_no_cross', 'v9_nocross', 'v9_no_sp', 'v9_nosp', 'v11_newA_no_cross', 'v11_newa_no_cross', 'v11_newA', 'v11_newa', 'v11_allA_no_cross', 'v11_alla_no_cross', 'v11_allA', 'v11_alla', 'v11_newA_cross', 'v11_newa_cross', 'v11_allA_cross', 'v11_alla_cross', 'v10_p_prior', 'v10_b', 'p_prior_residual', 'v10_audio_p_prior', 'v10_c', 'audio_p_prior'])
