...
def train_one_model(
    ...
    train_ds = ElderV3Dataset(
        train_rows, cfg.train_data_root, p_map, p_embed_map, m_map, scalers=scalers,
    ...
    val_ds = ElderV3Dataset(
        val_rows, cfg.train_data_root, p_map, p_embed_map, m_map, scalers=scalers,
        audio_features=audio_features, official_video_features=official_video_features,
...
def train_command(args: argparse.Namespace) -> None:
    ...
    print(f"[INFO] P_struct={len(p_map)} cols={len(p_cols)} P_embed={len(p_embed_map)} motion={len(m_map)}")
    ...
    for fold, (tr_idx, va_idx) in enumerate(skf.split(ids_all, y), start=1):
        ...
        for seed in seeds:
            res = train_one_model(cfg, train_rows, val_rows, p_map, p_embed_map, m_map, fold, seed, exp_dir)
...
def predict_one_checkpoint(
    ...
    cfg = ckpt["config"]
    ds = ElderV3Dataset(
        test_rows, test_data_root, p_map, p_embed_map, m_map, scalers=scalers,
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--output_dir", required=True)
    p.add_argument("--model_arch", default="lite", choices=['lite', 'p_anchor_v4', 'anchor', 'p_anchor', 'v5_hier_ord', 'v5', 'hier_ord', 'v6_cross_motion', 'v6_cross', 'cross_motion', 'v6_cross_no_pgate', 'v6_no_pgate', 'v6_cross_no_crossgate', 'v6_no_crossgate', 'v7_res_hier', 'v7_hier', 'v7', 'v9_depformerv2', 'v9', 'depformerv2', 'v9_no_pguide', 'v9_no_p', 'v9_no_cross', 'v9_nocross', 'v9_no_sp', 'v9_nosp'])
